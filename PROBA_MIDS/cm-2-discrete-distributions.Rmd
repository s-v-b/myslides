...
success parameter $p \in [0,1]$,

then

$$Y = \sum_{i=1}^n X_i$$

is distributed according to a Binomial disctribution with size parameter $n$ and success probability $p$:
...
By linearity of expectation, the expected value of the binomial distribution with parameters $n$ and $p$ is $n \times p$

The variance of a sum of independent random variables  is the sum of the variances

The variance of the binomial distribution with parameters $n$ and $p$ is $n \times p(1-p)$.
...
```{r witbinom, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='(ref:witbinom)', fig.align="center"}
# %%
n <- 20
...
$$X ⊥\!\!\!\bot Y, \quad X \sim\operatorname{Bin}(n_1, p), \quad Y \sim\operatorname{Bin}(n_2, p) \Rightarrow X+ Y \sim\operatorname{Bin}(n_1+n_2, p)$$
