...
under countable set-theoretical operations.
...
It is denoted by $\mathcal{B}(\mathbb{R})$.
...
`r fontawesome::fa("brain")`  Consider the $\sigma$-algebra generated by open-intervals of $\mathbb{R}$ with rational bounds. Is it the Borel $\sigma$-algebra?
...
`r fontawesome::fa("hand-point-right")` A positive measure $\mu$ is not necessarily a probability distribution.

For example,
the _counting measure_ $\mu$ on $\mathbb{N}$  satisfies
...
In this sense, the uniform distribution is the mother of all distribution.

---

An outcome $\omega$ of the uniform distribution is a real number. How does a
typical outcome look? A real number $\omega \in [0,1]$ has binary expansions:
...
`r fontawesome::fa("brain")` Do we really to assume that the function $F$ has left-limits in Theorem?
...
Hence, it makes sense to talk about the smallest $\lambda$-system containing a collection of sets.
