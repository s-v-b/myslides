...
`r fontawesome::fa("hand-point-right")`  Up to now, we have not made any use of the fact that $Z$ is a function of
...
`r fontawesome::fa("syringe")` For any real-valued random variable $X$,
...
`r fontawesome::fa("hand-point-right")` the infimum is achieved whenever $Z_{i}=\mathbb{E}^{\left(i\right)}Z$.
...
`r fontawesome::fa("point-hand-right")` If $Y_1,\ldots,Y_p \sim_{\tetx{i.i.d.}} \mathcal{N}(0,1)$ then $\sum_{i=1}^p Y_i^2  \sim \chi^2_p$
