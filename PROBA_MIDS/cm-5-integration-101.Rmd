...
a subclass of piecewise measurable functions in Section \@ref(simplefunctions). Defining the integral of a simple function
with respect to a measure in Section \@ref(integration) is straightforward. Some more work allows us to derive
useful properties: linearity, monotonicity, to name a few. In section \@ref(integration), we define
...
Section \@ref(biglimittheorems) states three convergence theorems
culminating with the _dominated convergence theorem_.

In Section \@ref(expectation), we relate the notion of _expectation_
of a random variable and the notion of integral. The _Transfer Theorem_
(Theorem \@ref(thm:transfertheorem)) is a key instrument in the
...
### Definition
simplefun, name="Simple function"}
...
### Definition
intsimplefun, name="Integral of a simple function"}
...
as $$\int_\Omega (f)_+ \mathrm{d}\mu - \int_\Omega (f)_- \mathrm{d}\mu$$
...
### Proposition
 limsimplefun, name="Approximation of measurable functions"}
...
for each $\omega \in \Omega$, the following holds:$$f_1(\omega) \leq f_2(\omega) \leq \ldots \leq f_n(\omega) \leq \ldots \leq f(\omega)$$
...
Figure \@ref(fig:approxexpsimple) illustrates the approximation capabilities of simple functions.


(ref:approxexpsimple) Approximation of the exponential function by simple functions $n \wedge \Big(2^{-n} \big\lfloor 2^n \exp(\omega) \big\rfloor \Big)$ for $n=2, 3, 4$.


```{r approxexpsimple, echo=FALSE, fig.cap='(ref:approxexpsimple)'}
# %%
...
$$\mu\Big\{ \omega: f(\omega)> g(\omega)\Big\} = 0 \, .$$ ($f$ is less of equal than $g$ $\mu$-almost everywhere), then
...
Theorems \@ref(thm:thmConvMonInt), \@ref(thm:thmfatou), \@ref(thm:thmdominatedConv) below
...
(beware there is no uniformity guarantee). We have $$\lim_n \uparrow E_n = \Omega\, .$$
...
Let $g = \sum_{i \leq k} c_i \mathbb{I}_{A_i}$ where $(A_i)_{i\leq k}$ is a finite partition of $\Omega$ into measurable subsets. $$\mathbb{I}_{E_n}  g =  \sum_{i \leq k} c_i \mathbb{I}_{A_i \cap E_n} \, .$$
...
This proves that \@ref(eq:monotone1) holds for all $a\in [0,1)$ and $g \in \mathcal{S}_+$ with $g \leq f$:
...
$$\int h_n \mathrm{d}\mu \uparrow \int f\mathrm{d}\mu$ and $\int j_n \mathrm{d}\mu \downarrow \int f\mathrm{d}\mu$$
...
$(\mathcal{Y}, \mathcal{G})$  a measurable space,
$f$ a measurable function from $(\mathcal{X}, \mathcal{F})$ to $(\mathcal{Y}, \mathcal{G})$.
...
It embodies the interaction between convexity and expectation.
...
```{r convexfunfig, echo=FALSE, fig.cap='(ref:convexfunfig)'}
# %%
foo <- function(x){
  ifelse(x < 0, -x, x^2)
}
...
```{r graphyoung, echo=FALSE, fig.cap='(ref:graphyoung)'}
# %%
...
Theorem \@ref(thm:thmcauchyschwartz) tells us that if $X$ and $Y$ are
...
Let $X, Y$ be two real-valued random variables defined on the same probability space. Let $p \in [1, \infty)$.

Assume that $\mathbb{E}|X|^p <\infty$  and $\mathbb{E}|Y|^p<\infty.$ Then we have:
...
by Jensen's inequality  (Theorem \@ref(thm:thmjensen)), we have
...
Proposition \@ref(prp:nestinglp) is a about inclusion of sets. The next theorem
