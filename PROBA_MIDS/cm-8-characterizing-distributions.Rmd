...
##  [Characteristic fucntions](#characteristic-functions)

## [Convolutions](#convolution)

## [Gaussian functions](#gaussian-characteristic)

## [Inversion Theorem](#inversion-theorem)
...
For _univariate distributions_, a companion to the cumulative distribution function  is the quantile function. It plays a significant role in simulations, statistics and
risk theory.
...
Cumulative distribution functions for the probability distributions illustrated in Figure \@ref(fig:quantiles2)
...
(ref:quantiles4) Representation of $F \circ F^{\leftarrow}$ for the probability distributions illustrated in Figures \@ref(fig:quantiles2)  and \@ref(fig:quantiles3). The function $F \circ F^{\leftarrow}$ always lies above the line $y=x$ (dotted line) as prescribed in Proposition \@ref(prp:quantilekit). Plateaux that lie strictly above the dotted line are in correspondence with  jumps of the quantile function.
...
.pull-right[
 ...
 This is a popular device for comparing two probability distributions on the real line defined by their cumulative distributions $F$ and $G$.

 The QQ-plot consists in plotting function $G^{\leftarrow} \circ F$.

 If the two distributions are the same, the line $y = G^{\leftarrow} \circ F(x)$ should lie below $y=x$ (according to Proposition \@ref(prp:quantilekit)), and should even coincide with the diagonal if $F=G$ is continuous and increasing.
