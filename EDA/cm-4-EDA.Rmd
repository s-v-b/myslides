...
```{r qplot-residuals-label, fig.show='hide', fig.height=4}
...
had whispered that all but `13` coefficients are null, and even told us the index
...
The null hypothesis asserts that the true vector of coefficients belong to a linear subspace of dimension `13` of $\mathbb{R}^{100}$. If we stick to Gaussian linear modelling
...
The rule of thumb when using the `p.value` reported by a test function is the following: if you are ready to be wrong _under the null hypothesis_ with probability $\alpha$, then you should reject when the `p.value` is less than $\alpha$.
