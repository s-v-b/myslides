...
tit %>%
  select(Pclass, Survived) %>% # Projection on two columns
  table()     #<<
```
...
```{r, echo=TRUE, results='asis', eval=FALSE, include=FALSE}
...
```{r gaussiancloud-sim, echo=TRUE, fig.show='hide', fig.height=4}
...
```{r flip-scatplot-residuals, include=FALSE}
p_scatter_gaussian %+%
  broom::augment(lm(Y ~ X, u)) +  #<<
  geom_line(aes(x=X, y=.fitted)) +
...
`r flipbookr::chunk_reveal("flip-scatplot-residuals", title="### Visualizing residuals")`
...
- `r fontawesome::fa("hand-point-right")` Different datasets can lead to the same regression line and the same `Multiple R-squared`
...
.panel[.panel-name[Anscombe in long format]
...
```{r pivot_anscombe, include=FALSE}
datasets::anscombe %>%
  pivot_longer(everything(),
    names_to = c(".value", "group"),
    names_pattern = "(.)(.)"
...
`r flipbookr::chunk_reveal("pivot_anscombe", title="### Pivoting Anscombe")`

???

Interpreting `pivot_longer`
