...
In the field of visualization, a landmark video.

The late _Hans Rosling_, is famous for a 2006 Ted talk called _The best statistics you’ve ever seen_.  It became one of the most seen TED talks ever, thanks to its unique combination of knowledge-testing, animating bubble charts and storytelling about global development.
...
What's in a data frame? What's in a tibble?
...
`glimpse()` allows to see the schema and the firts rows
...
In the parlance of Relational Algebra, this is a _selection_
...
We build a graphical object (a `ggplot` object) around a data frame. We supply aesthetic mappings that can be either global
or bound to some geometries or statistics. The global aesthetic
mapping defines which columns are mapped to which axes, possibly mapped to colours, linetypes or shapes. Geometries and Statistics
describe the building blocks of graphics.
...
- When is it important to use logarithmic scaling on some axis?
...
- [Getting the theme](https://ggplot2.tidyverse.org/reference/theme_get.html)
...
p <- p + labs(title=glue("Gapminder {min(p$data$year)}-{max(p$data$year)}"))
