...
The acclaimed TED presentation by [Hans Rosling](https://en.wikipedia.org/wiki/Hans_Rosling) illustrates the Grammar of Graphics approach

---
### Grammar of Graphics in Action  `r fontawesome::fa("video")`
...
[Gapminder](https://www.gapminder.org) is a Swedish foundation. 
...
The TED presentation is an example of attractive data visualization
...
- How can we analyze/explore data?

- How can we present data (to a diverse audience)? 

---
template: inter-slide
name: diy

## Do It Yourself with `r fontawesome::fa("r-project")`
...
In `r fontawesome::fa("r-project")` The `gapminder` data can be loaded from *package* `gapminder`

- `r fontawesome::fa("download")` Packages can be *installed* from `CRAN` using `install.packages()`

- `r fontawesome::fa("hdd")` Once a package is *installed* on the hard drive, it has to be *loaded* in the current session using `require()` or `library()`
...
We pick two random rows for each continent
...
-  No/Few redundancies (what does it mean)
  ...
  -  ....
-  Compare with Codd's principles from database theory
...
[Package `dplyr` docs](https://www.rdocumentation.org/packages/dplyr/versions/0.7.8/topics/filter)
...
We supply _aesthetic mappings_ (`aes()`) that can be either global or bound to some _geometries_ (`geom_point()`) or _statistics_ 
...
Aesthetic mappings are usually complemented by, possibly implicit, *scales* 
...
p_titre <- glue("Gapminder {min(p$data$year)}-{max(p$data$year)}")

p_caption <- "From sick  and poor (bottom left) to healthy and rich (top right)"
...
### A static plot for Gapminder (2002)

```{r theme_scale-out-bis, warning=FALSE, echo=FALSE, message=FALSE, out.width="90%"}
p
```
...
```{r plot_gap_minder_2002_labelled, eval=FALSE, fig.cap="Gapminder 2002 layer by layer"}
...
```{r plot_gap_minder_2002_labelled-out, ref.label="plot_gap_minder_2002_labelled", warning=FALSE, echo=FALSE, message=FALSE}
...
- `facet_zoom()` on a continent (from `ggforce`)
