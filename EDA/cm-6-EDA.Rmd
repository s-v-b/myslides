...
Some datasets have non-numerical columns. Such columns are not used when computing the SVD that underpins PCA. But non-numerical columns may be incorporated into PCA visualization
...
standardize <- function(df)
  mutate(df, across(where(is_numeric),
                    ~ .x /sd(.x)))

center <- function(df)
  mutate(df, across(where(is_numeric), ~ .x - mean(.x)))

scale_verse <- function(df)
  mutate(df, across(where(is_numeric), ~ (.x - mean(.x))/sd(.x)))
```
]
]
---
...
microbenchmark(sweep(iris[,-5], 2, STATS = apply(iris[,-5], 2, sd), FUN = "/"))

microbenchmark(mutate(iris, across(where(is.numeric),
                                   ~ (. - mean(.))/ sd(.))))  #<< dplyr based

microbenchmark(scale(iris[, -5]))
```
...
total_inertia <- sum(pca_iris$sdev^2)
...
.fl.w-50[
```{r decathlon_pcs_plots, fig.show='hide'}
