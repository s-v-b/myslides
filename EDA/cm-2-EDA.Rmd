...
output:
  xaringan::moon_reader:
    css: ["header-footer.css",  "conf/css/style.css", "conf/css/fonts.css"]
    lib_dir: libs
    seal: false
    includes:
      in_header:
...
- Mean: $\overline{X}_n = \frac{1}{n}\sum_{i=1}^n x_i$  (location)

- Standard deviation: $s^2 = \frac{1}{n-1} \sum_{i=1}^n \big(x_i - \overline{X}_n\big)^2$  or $s^2 = \frac{1}{n} \sum_{i=1}^n \big(x_i - \overline{X}_n\big)^2$  (dispersion)

- Skewness   $\frac{\frac{1}{n} \sum_{i=1}^n \big(x_i - \overline{X}_n\big)^3}{s^3}$

- Kurtosis $\kappa = \frac{\frac{1}{n} \sum_{i=1}^n \big(x_i - \overline{X}_n\big)^4}{s^4} - 2$
...
In descriptive statistics, a box plot or boxplot is a method for graphically depicting groups of numerical data through their _quartiles_.
...
- For a Gaussian population, this leads to
$1.5 \text{IQR} = 3 \sigma \Phi^{\leftarrow}(3/4) \approx 2.02 \sigma \approx \sigma \Phi^{\leftarrow}(.98)$.
...
```{r normal-boxplot-label, message=FALSE, warning=FALSE, fig.show='hide'}
sample <-  rnorm(1000)
title <- stringr::str_c("Standard Gaussian sample, size : ",
                        length(sample))
qplot(y=sample, x=1L) +
  geom_boxplot() +  #<<
  ggtitle(title) +
...
If we _boxplot_ a Gaussian sample, on average, $4\%$ of the points lie outside the
whiskers. Such points are called _outliers_

Boxplots provide a visual assessment of the departure of a sample from normality.
...
`r fontawesome::fa("database")`  Table wrangling
...
`r fontawesome::fa("")` Base function `summary` tells us everything there is to know: number of passengers for every class and passengers, and number of sample elements  with unknown class 
...
```{r normalize-col-plot-label, fig.show='hide'}
dt <- dt %>%
...
```{r, echo=FALSE}
(p + geom_col(width=.3)) %+% dt[dt$Cat != 'Crew',]
```
...
[Filip Semmelweis 1818-865](https://en.wikipedia.org/wiki/Ignaz_Semmelweis)
]
]
...
This fact was known outside the hospital. The two clinics admitted on alternate days, but women begged to be admitted to the Second Clinic, due to the bad reputation of the First Clinic.
...
Data wrangling makes plotting easy.

```{r semmelweis-label, fig.show="hide"}
tmp %>%
