...
```{r loading, eval=F, echo=T}
require(tidyverse, quietly = T)

require(nycflights13, quietly = T)
...
```{r carrier-out,  echo=T, eval=T}
summary(flights$carrier)

summary(factor(flights$carrier))
```

`r fontawesome::fa("lightbulb")` Base function `summary` tells us everything there is to know, counts, and NAs

`r fontawesome::fa("exclamation-triangle")` Base function `table` puts dust under the carpet: NAs are not reported 
]
...
dt %>%
  ggplot() +
  aes(x=fct_reorder(factor(dest), Prop, .desc = T),   #<<
      y=Prop) +
  ggtitle("Flights: dest distribution") +
  geom_col(width=.3) +
  theme(axis.text.x=element_text(angle=45, hjust=1, size =4))  +
