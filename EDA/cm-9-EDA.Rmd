...
A hierarchical clustering of $\mathcal{X}$ is a sequence of  $|\mathcal{X}| -1$ nested partitions 
of $\mathcal{X}$, starting from the trivial partition into  $|\mathcal{X}|$ singletons and ending 
into the trivial partition in $1$ subset ( $\mathcal{X}$ itself)

A hierarchical clustering consists of $|\mathcal{X}|-1$ flat clusterings
...
rownames(iris_num) <- str_c(iris$Species, 1:150, sep = "_")
