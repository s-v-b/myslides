...
iris_num <- dplyr::select(iris, where(is.numeric))
...
tmp <- dendro_iris %$% {
  list("segments"= segments,
       "labels"= labels %>% 
         mutate(couleur=map_chr(label, ~ str_match(., '^([\\w]+)_[0-9]+')[1,2]),
                label= map_chr(label, ~str_match(., '^[\\w]+_([0-9]+)')[1,2])))
} 
