...
##  `r fontawesome::fa("fingerprint")` Indicator and Burt matrices
...
```{r, results='hide'}
tol <- 1e-10
X <- select(tit, Pclass, Sex, Embarked, Age)
p <- ncol(X)
...
```{r, results='hide'}
assert_that(norm(abs(t(svd.SS$u[,1:8]) %*% diag(sqrt(r)) %*%  res.mca$svd$U) - diag(1, 8), "F") <= tol)
```
...
### Plotting colums from data matrix
...
fviz_mca_ind(res.mca.2, 
             geom= "point", habillage = 'Survived') +
  coord_fixed() +
  ggtitle("MCA Titanic")


fviz_mca_biplot(res.mca.2,
                geom.ind="point", 
                repel=TRUE)
plot(res.mca, choix = "ind", col.ind.sup = "none", invisible = "ind")
...
res.mca.2$ind$coord %>%
  ...
  drop_na() %>% 
  ggplot() +
  aes(x=`Dim 1`, y=`Dim 2`, colour=Survived) +
