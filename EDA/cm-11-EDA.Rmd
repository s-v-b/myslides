...
- Some variables may be called explanatory, one variable may be considered as a response variable. Is the response variable
dependent on the explanatory variables? If yes, we may wonder whether, conditionally on some explanatory variabless, the response variable is independent on the other explanatory variables
...
In this session, we will adress the different settings
...
- we may investigate response/attitudinal variables with respect to  explanatory/demographic variables
- we may investigate collections of  response/attitudinal variables
- we may investigate collections of explanatory/demographic variables
...
### `r fontawesome::fa("ship")` Flattening Titanic table
...
`Titanic` is a base `r fontawesome::fa("r-project")` contingency table where surn
---
exclude: true
...
`Survived` is considered as a _supplementary_ variable: it may projected on the factorial subspaces
defined by the four columns `Sex`, `Age`, `Embarked`, `Pclass`.

`Survived` plays no role in the definition
of the factorial sub-spaces (while computing the svd)
...
### The output of  ` MCA ` : `print(mca.tit)`
...
### The output of `MCA`: examining `$call` (intermediate results)

- `mca.tit$call$Xtot` is the _supplemented_ indicator matrix
- `mca.tit$call$Xtot$marge.row` is a constant vector
- `mca.tit$call$Xtot$marge.row` is proportional to the means of the columns of the indicator matrix
-
...
`print(mca.tit)` bypasses an important part of the list: `mca.tit$svd`


```{r}
U <- mca.tit$svd$U   # orthogonal columns
V <- mca.tit$svd$V   # orthogonal w.r.t.
D <- diag(mca.tit$svd$vs[1:5])

dim(U)
(U %*% D %*% t(V)) - scale(mca.tit$call$Xtot[,-c(13, 14, 15)], center = TRUE, scale=FALSE)

mca.tit$svd$vs

colnames(mca.tit$call$Xtot)
colSums(U %*% D %*% t(V))
...
$(D_r^{-1/2} \times U)\times D \times (D_c^{-1/2}\times V)^T$ is the _extended SVD_ of $D_r^{-1} \times P \times D_c^{-1} - \mathbf{I}\mathbf{I}^T$ with respect to $D_r$ and $D_c$


---


### CA on incidence matrix

`r fontawesome::fa("hand-point-right")`

- The row wise  sums of an incidence matrix are equal to the number of variables of the data matrix

- The column wise  sums of an incidence matrix are equal to the numbers of occurrences of the corresponding levels
...
### Factor map: categories representation
...
.fl.w-40.pa2.f6[
```{r}
knitr::kable(get_eigenvalue(mca.tit), digits=2)
...
.fl.w-60.pa2.f6[
```{r}
fviz_screeplot(mca.tit, addlabels=TRUE)
```
]
...
```{r}
gg_mca_screeplot <- function(res.mca){
  p <- get_eigenvalue(res.mca) %>%
    as_tibble() %>%
    rownames_to_column(var="Dimensions") %>%
    ggplot() +
    aes(x=Dimensions, y = variance.percent, label=round(cumulative.variance.percent,2)) +
    geom_col() +
    scale_x_discrete() +
    geom_text(nudge_y=.5)
  p
...
```{r}
# fviz_mca_biplot(mca.tit, repel=TRUE, ggtheme=theme_minimal())
# fviz_mca_var(mca.tit)

# res.mca <- as.data.frame(tit)  %>%
#  select(Sex, Pclass, Embarked, Age) %>%
#  MCA(graph=FALSE)

# res.mca$var$coord %>% glimpse()
# fviz_mca_biplot(res.mca, geom="point")
...
```{r}
rownames(mca.tit$call$X)
```


---

### MCA results for variables
