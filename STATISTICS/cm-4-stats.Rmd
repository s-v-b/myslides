...
### Modéles exponentiels

### Performances de la méthode des moments
...
n peut vérifier que les gaussiennes univariées s'inscrivent dans ce schéma:
...
$T_1(x)= x, T_2(x) = - x^2$, $\theta =(\mu/\sigma^2, 1/(2 \sigma^2))^t$,
...
La convexité joue un rôle essentiel dans l'étude des modèles exponentiels, mais aussi dans la mise en oeuvre
...
Un modèle exponentiel est minimal ou de plein  rang si et seulement si
...
si et seulement si il est identifiable
...
Ce qui signifie que
$$\nu$-pp, $\langle \theta- \theta', T(x)\rangle$$
...
Ce calcul fournit une preuve concise de la relation entre
...
D'après  le _théorème d'inversion locale_:
