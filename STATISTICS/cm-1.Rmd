...
- La _moyenne empirique_

$$\overline{X}_n:=  \frac{1}{n} \sum_{i=1}^n x_i$$

- La  _variance empirique_
...
`r fontawesome::fa("hand-pointing-right")` Dans de nombreux développements des statistiques, par exemple en _estimation de densité_, on travaille sur des modèles plus riches qui n'admettent pas de paramétrisation _naturelle_ par une partie d'un espace euclidien de dimension finie
...
une suite d'estimateurs $(\widehat{\theta}_n)$ est
...
Pour notre problème jouet, la suite d'estimateurs $(\overline{X}_n)$ est fortement consistante pour l'estimation de $\theta$ (loi forte des grands nombres) `r fontawesome::fa("glass-cheers")`


On peut aussi vérifier que la suite $(\overline{X}_n/(1-\overline{X}_n))$ est fortement consistante pour l'estimation de $\theta/(1- \theta)$.
...
un _intervalle  de niveau de   confiance asymptotique $1- \alpha$_ avec $\alpha \in ]0,1[$
...
Si  $(X_n)_n$ et $(X_n)_n$ est deux  suites de variables aléatoires
...
`r fontawesome::fa("")`On peut se demander pourquoi on emploie l'expression _on ne rejette pas l'hypothèse nulle $H_0$_, plutôt que _on accepte l'hypothèse nulle_.
