...
output:
  xaringan::moon_reader:
    css: ["header-footer.css", "xaringan-themer.css", "hygge"]
    lib_dir: libs
    seal: false
    includes:
      in_header:
...
Une _statistique suffisante_ $T$  est _minimale_ si pour tout autre statistique suffisante $T'$, $T$ est une fonction de $T'$.
...
Dans un modèle, une statistique dont la loi est libre d'un paramètre est dite _ancillaire_ par rapport à ce paramètre
...
Dans le modèle gaussien $\{ \mathcal{N}(\mu, \Sigma): \mu \in \mathbb{R}^d, \Sigma \in  \text{DP}(d) \}$, la covariance empirique est ancillaire par rapport à l'espérance
...
Si $T$ est une _statistique suffisante minimale_ et si le modèle est _complet pour la statistique_ alors $T$ est indépendante de toute _statistique ancillaire_
...
où le paramètre inconnu est  $\theta_0$ dans $\mathbb{R}^p$. Selon les circonstances, on connait ou on ne connait pas $\sigma$
...
Ses coefficients diagonaux sont appelés (en anglais) _leverage_ et parfois traduits en français par _influence_ ou _effet levier_

---

Comme $\mathbf{X}$ est de plein rang, il existe un unique optimum, la solution de $\widehat{y} = \mathbf{X} \theta$
...
L'estimateur  des moindres carrés $\widehat{\theta}$ est un estimateur _sans biais_ `r fontawesome::fa("glass_cheers")`
...
Dans le cadre des _modèles linéaires gaussiens avec bruit homoschédastique_, l'estimation de la variance du bruit $\sigma^2$ est grandement facilitée `r fontawesome::fa("glass_cheers")`

Et la construction de région de confiance pour $\theta_0$ et $\sigma^2$  est immédiate `r fontawesome::fa("glass_cheers")`
...
$$\widehat{Y}- Y= \sigma (\mathbf{H}-\textbf{Id}) \epsilon$ et $\mathbf{X}(\widehat{\theta} - \theta_0)=  \sigma \mathbf{H} \epsilon$$

Au facteur $\sigma$ près, on obtient $\widehat{Y}- Y$ et $\mathbf{X}(\widehat{\theta}-\theta_0)$ en projetant le vecteur gaussien standard $\epsilon$ sur deux sous espaces orthogonaux de $\mathbb{R}^n$
...
$F_{p,n-p}$. Le fait que cette quantité aléatoire soit pivotale  conduit à une région de confiance pour $\theta_0$.
...
est plus grande que le quantile d'ordre $1- \alpha$ de la loi $F_{p-k,n-p}$ est de niveau $\alpha$
(la probabilité d'une erreur de première espèce est égale à $\alpha$).

L'étude de la puissance de ce test (la probabilité de rejeter $\text{Hyp}_0$  lorsque les données sont tirées sous l'alternative $\text{Hyp}_1$) est plus délicate.

On peut cependant vérifier que le  test est _sans biais_, c'est à dire que la probabilité de rejeter $\text{Hyp}_0$  dans ce cas est supérieure à $\alpha$.

---

Choisissons une loi relevant de l'alternative. Cela revient à choisir $\theta^0 \in \mathbb{R}^k$
...
Les vecteurs aléatoires $\widehat{Y} - \widehat{Y}^0$ et $Y - \widehat{Y}$  restent indépendants, mais  la statistique

$${\|\widehat{Y} - \widehat{Y}^0\|^2/\sigma_0^2}$$

est maintenant distribuée selon une loi du $\chi^2$ à $p-k$ degrés de liberté décentrée de
$\|(\textbf{Id}-\mathbf{H}^0) \mathbf{X}^1 \theta^1 \|_2$.

La quantité aléatoire   ${\| \widehat{Y} - {Y}\|^2/\sigma_0^2}$ reste distribuée selon $\chi^2_{n-p}$.

Pour établir que la loi de $S$ sous $\text{Hyp}_1$ domine  stochastiquement la loi de $S$ sous $\text{Hyp}_0$, il suffit de se rappeler que à nombres de degrés de liberté égaux, la loi du  $\chi^2$ décentrée domine stochastiquement la loi du $\chi^2$ centrée.
...
`r fontawesome::fa("")` La fonction `lm` de `r fontawesome::fa("r-project")`
