...
params:
  title: "VII : Tests de type Chi-deux"
...
`r fontawesome::fa("ship")` L'échantillon a-t-il été engendré par $n$ tirages indépendants
...
La distribution du carré de la norme d'un vecteur gaussien dont la matrice de covariance est $\sigma^2 \operatorname{Id}$.
dépend de la norme de l'espérance mais ne dépend pas de sa direction

Cette distribution peut être comparée _stochastiquement_ à la distribution du carré de la norme d'un vecteur gaussien centré de  même covariance
...
Dans un espace probabilisé de loi $\mathbb{P}$, une variable aléatoire réelle   $X$  est _stochastiquement inférieure_ à une  variable aléatoire  $Y$, si

$$\mathbb{P} \{ X \leq Y \} =  1$$

On dit que la distribution de $Y$  domine stochastiquement la  distribution de  $X$
...
Si  $X$  est  stochastiquement  inférieure à  $Y$  et si   $F$  et  $G$  désignent les fonctions de répartitions de $X$ et $Y$, alors pour tout  $x \in  \mathbb{R}$, $F(x)\geq G(x)$. 

Les fonctions quantile  $F^\leftarrow, G^\leftarrow$  satisfont $F^\leftarrow(p) \leq G^\leftarrow(p)$ for $p \in (0,1)$
...
Si  $F$ et $G$  sont  deux fonctions de répartition qui satisfont  $\forall x \in \mathbb{R}$ $F(x)\geq G(x)$

alors

il  existe une espace probabilisé équipé d'une loi  $\mathbb{P}$ et 
deux  variables aléatoires  $X$ et $Y$  avec des fonctions de répartition $F, G$  qui satisfont:
...
On équipe  $([0,1], \mathcal{B}([0,1])$  avec la loi uniforme

Soient $X (\omega)=  F^{\leftarrow}(\omega)$,  $Y(\omega) = G^\leftarrow(\omega)$.

La loi de $X$  (resp. $Y$) a pour fonction de répartition $F$ (resp. $G$) et on a:
...
où  $Z_i$  sont  i.i.d. selon $\mathcal{N}(0,\sigma^2)$.
...
La loi de $\| Y\|^2/\sigma^2$ ($\chi^2$ décentrée de  paramètre $\| \theta\|_2/\sigma$)
_domine stochastiquement_ la loi $\| X\|^2/\sigma^2$ ( $\chi^2$ centrée de avec le même nombre de degrés de liberté)
...
Le vecteur gaussien $Y$ est  distribué comme  $\theta + X$.   Il existe une transformation orthogonale
$O$ telle que 

$$O \theta = \begin{pmatrix} \| \theta\|_2 \\ 0 \\ \vdots \\ 0\end{pmatrix}$$

Les vecteurs  $OY$ et  $OX$  ont respectivement les mêmes normes que  $X$ et $Y$.

Le carré de la norme de  $Y$
est distribué comme  le carré de la norme de $OY$, soit comme $(Z_1 + \|\theta\|_2)^2 +\sum_{i=2}^d Z_i^2$.
...
Pour  $y>0$, la fonction de $[0,\infty)$ vers $\mathbb{R}$, définie par 

$$a \mapsto \Phi(y-a) - \Phi(-y-a)$$

est décroissante par rapport à $a$
...
La dernière étape de la preuve se lit comme suit

$$\mathbb{P} \left\{ X \in \theta + C \right\} \leq \mathbb{P} \left\{ X \in C\right\}$$

où  $X \sim \mathcal{N}(0,\operatorname{Id}_1)$, $\theta \in \mathbb{R}$ and $C = [-\sqrt{x},\sqrt{x}]$

Cette inégalité  vaut en dimension $d\geq 1$ si   $C$ est compact, convexe, symétrique
...
Pour chaque $n$, $S_n$  est une somme de variables aléatoires indépendantes identiquement distribuées, centrées. 

La variance de $S_n$  est $\sigma^2_n$. 

La distribution des variables aléatoires
dont la somme centrée et normalisée donne $S_n$  dépend de $n.$ 

On dit qu'on a affaire à un __tableau triangulaire__
...
On  vérifie que les conditions sont remplies dans le cas des vecteurs multinomiaux
...
La loi du $\chi^2$  décentrée domine stochastiquement la loi du $\chi^2$ centrée

Sous la suite $P_{\theta_n}$
le test du $\chi^2$ de niveau $\alpha$  est de puissance asymptotique supérieure ou égale à $\alpha$ 

On dit que le test est (asymptotiquement) _sans biais_
